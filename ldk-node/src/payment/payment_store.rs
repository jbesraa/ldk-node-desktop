...
impl Readable for PaymentDetails {
	fn read<R: lightning::io::Read>(reader: &mut R) -> Result<PaymentDetails, DecodeError> {
		...
		let user_token: String = user_token.0.ok_or(DecodeError::InvalidValue)?;
		...
		Ok(PaymentDetails { id, kind, amount_msat, direction, status, user_token })
	}
}
