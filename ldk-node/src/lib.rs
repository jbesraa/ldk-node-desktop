...
impl<K: KVStore + Sync + Send + 'static> Node<K> {
	...
	/// Return the network this node is running on.
	...
	/// Return the network this node is running on.
	pub async fn height(&self) -> Result<u32, esplora_client::Error> {
		self.wallet.height().await
	}
	...
	pub fn spendable_onchain_balance_sats(&self) -> Result<u64, Error> {
		...
		dbg!(&balance);
		Ok(balance.map(|bal| bal.get_spendable())?)
	}
